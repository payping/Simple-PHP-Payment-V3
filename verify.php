<?php
/* PaymentRefId: این فیلد تنها در صورتی که پرداخت موفق باشد ارسال خواهد شد. */
if($_POST['Status'] == -1 ) {
    /* در صورتی که PaymentRefId وجود نداشت از verify جلوگیری میشود. */
    $msg = 'عدم پرداخت توسط پرداخت کننده';
} else {
    ...
    try{
        ...
        if($err){
            ...
            if($header['http_code'] == 200){
                ...
                if( isset($PaymentRefid) and $PaymentRefid != '' ){
                    $msg = ' تراکنش موفق بود: ' . $PaymentRefid;
                    $outp['msg'] = $msg;
                }else{
                    $msg = 'متاسانه سامانه قادر به دریافت کد پیگیری نمی‌باشد! نتیجه درخواست: ' . $header['http_code'];
                }
